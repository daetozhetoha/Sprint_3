...
def test_personal_area_enter_after_click_button_success():
    options = Options()
    ...
    WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located(TestLocators.ACCOUNT_PROFILE_INFORMATION))
    assert driver.find_element(*TestLocators.ACCOUNT_PROFILE_INFORMATION).text == "Профиль"
    driver.quit()
